...
<div class="page-header">
  <h4 class="page-title">{{ __('Recharge Package Management') }}</h4>
  <ul class="breadcrumbs">
    ...
    <li class="nav-item">
      <a href="#">{{ __('Recharge Packages Management') }}</a>
    </li>
  </ul>
</div>
  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <div class="row">
          <div class="col-lg-4">
            <div class="card-title d-inline-block">{{ __('Recharge Packages') }}</div>
          </div>
      ...
      <div class="card-body">
        <div class="row">
          <div class="col-lg-12">
            @if (count($packages) == 0)
            <h3 class="text-center">{{ __('NO RECHARGE PACKAGE FOUND') . '!' }}</h3>
            @else
                  ...
                  <tr>
                    ...
                    <td>
                      <form id="FeatureStatusForm-{{ $package->id }}" class="d-inline-block"
                        action="{{ route('admin.package.recharge.update_package_featured_status', ['id' => $package->id]) }}"
                        method="post">
                        @csrf
                        <select
                    ...
                    <td>
                      <form id="ActiveStatusForm-{{ $package->id }}" class="d-inline-block"
                        action="{{ route('admin.package.recharge.update_package_active_status', ['id' => $package->id]) }}"
                        method="post">
                        @csrf
                        <select
                    ...
                    <td>
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                          ...
                          <li>
                            <a class="dropdown-item"
                              href="{{ route('admin.package.recharge.invest_edit_package', ['package'=>$package->id, 'language' => $language->id]) }}">
                              Edit
                              (in
                              {{ $language->name }})
                            </a>
                      ...
                      <form class="deleteForm d-inline-block"
                        action="{{ route('admin.package.recharge.delete_package', ['id' => $package->id]) }}" method="post">
                        @csrf
...
{{-- create modal --}}
@include('backend.recharge-packages.create')
@endsection
