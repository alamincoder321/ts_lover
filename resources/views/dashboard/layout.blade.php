...
<head>
    ...
    {{-- title --}}
    <title>{{ __('Subscription Dashboard') . ' | ' . $websiteInfo->website_title }}</title>
...
<body data-background-color="{{ Session::get('vendor_theme_version') == 'light' ? 'white2' : 'dark' }}">
    ...
    <div class="wrapper">
        ...
        @includeIf('dashboard.partials.top-navbar')
        {{-- top navbar area end --}}

        {{-- side navbar area start --}}
        @includeIf('dashboard.partials.side-navbar')
    ...
    {{-- include scripts --}}
    @includeIf('dashboard.partials.scripts')
